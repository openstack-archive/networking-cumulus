    ...
    def delete_network(self, context, current):
        ...
        with netconf.ConfFile(netconf.INT_BRIDGE) as cfg:
            cfg.ensure_opt_not_contain_value('bridge-vids',
                                             str(segmentation_id))
        LOG.info("Network with vlan %(segmentation_id)d was deleted.",
                 {'segmentation_id': segmentation_id})
...
class CumulusRpcClientAPI(object):
    ...
    def _get_cctxt(self, host=None, fanout=False):
        return self.client.prepare(
            version=self.ver, topic=self._get_device_topic(host=host),
            fanout=fanout)

    def create_network_cast(self, current):
        return self._get_cctxt(fanout=True).cast(self.context, 'create_network',
                                          current=current)

    def delete_network_cast(self, current):
        return self._get_cctxt(fanout=True).cast(self.context, 'delete_network',
                                          current=current)
